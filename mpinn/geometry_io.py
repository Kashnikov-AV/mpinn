...
def load_stl(filepath: str, compute_normals: bool = True) -> MeshData:
    ...
    with open(filepath, "rb") as f:
        ...
        return MeshData(vertices=vertices, faces=faces, normals=normals, dim=3)
    ...
    if compute_normals:
        if vertex_normals and face_normal_indices:
            ...
            for i, fn_indices in enumerate(face_normal_indices):
                face_normal = np.mean(
                    [vertex_normals[idx] for idx in fn_indices], axis=0
                )
...
def load_gmsh(filepath: str) -> MeshData:
        ...
        if line == "$Nodes":
            ...
            if len(first_elem_line) >= 4:
                ...
                for _ in range(n_elements):
                    ...
                    node_ids = [
                        int(x) - 1 for x in parts[3 + n_tags :]
                    ]  # 1-based to 0-based
    ...
    faces = (
        np.array(faces_2d, dtype=np.int32)
        if faces_2d
...
def load_geometry(filepath: str, file_format: str | None = None, **kwargs) -> MeshData:
    ...
    if file_format == "stl":
        ...
        raise ValueError(
            f"Unsupported file format: {file_format}. Supported: stl, obj, msh"
        )
...
def sample_points_on_surface(
    ...
    normals = (
        jnp.array(mesh.normals, dtype=jnp.float64) if mesh.normals is not None else None
    )
    ...
    if normals is not None:
        ...
        sampled_normals = (
            n0_sel + u1[:, None] * (n1_sel - n0_sel) + u2[:, None] * (n2_sel - n0_sel)
        )
        sampled_normals = sampled_normals / (
            jnp.linalg.norm(sampled_normals, axis=1, keepdims=True) + 1e-10
        )
    else:
        # Use face normals
