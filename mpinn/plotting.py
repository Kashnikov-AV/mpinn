...
def save_plot(
    x_test, T_pred, T_exact, phys, save_path, title="Сравнение PINN и точного решения"
):
    """
...
def show_history(
    ...
    # Граничные условия
    plt.semilogy(
        steps, history["bc_0"], "r--", label="ГУ слева", linewidth=2, alpha=0.7
    )
    plt.semilogy(
        steps, history["bc_1"], "r--", label="ГУ справа", linewidth=2, alpha=0.7
    )
    # Общая ошибка
    plt.semilogy(
        steps,
